...
class GateAttntionModel(nn.Module):
    ...
    def __init__(self, g, input_size, hidden_size, output_size, nonlinearity):
        ...
        intern_num_heads = 4
        final_num_heads = 6
        self.gat1 = GATConv(input_size, hidden_size, intern_num_heads, activation = nonlinearity)
        self.gat2 = GATConv(hidden_size * intern_num_heads, hidden_size, intern_num_heads, activation= nonlinearity)
        self.gat3 = GATConv(hidden_size * intern_num_heads , output_size, final_num_heads )

    def forward(self, inputs):
        outputs = self.gat1( self.g, inputs )
...
def main(args):
    ...
    model = GateAttntionModel(g=train_dataset.graph, input_size=n_features,
